...
const RemindersAndAlerts = () => {
  return (
    <div className="bg-gray-900 text-white p-8 min-h-screen w-[calc(100vw-300px)] flex flex-col items-center overflow-hidden">
      <motion.div 
        initial={{ opacity: 0, y: -20 }} 
        animate={{ opacity: 1, y: 0 }} 
        transition={{ duration: 0.6 }}
        className="bg-gray-800 p-6 rounded-xl shadow-2xl w-full max-w-2xl mt-10 border-2 border-[#00B6BA] flex flex-col gap-6 transition-transform transform hover:scale-105"
      >
        <h3 className="text-3xl font-bold text-[#00B6BA] text-center tracking-wide">
          🔔 Reminders & Alerts
        </h3>
        ...
        {reminders.map((reminder, index) => (
          <motion.div
            ...
            className={`flex items-center gap-4 p-5 rounded-lg shadow-md transition-transform transform hover:scale-105 ${
          ...
          >
            ...
            <div>
              ...
              <p className="text-gray-300 text-md">{reminder.time}</p>
            </div>
          </motion.div>
        ))}
      </motion.div>
